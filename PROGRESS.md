...
图例：✅ 已实现 · 🚧 进行中 · ⬜ 未开始 · 💡 需求池（已接受 · 待排）
优先级（未完成项）：[P0] V1 毕业必需 · [P1] V1.x · [P2] V2+/远期
...
- ⬜ 构件筛选（G15）：交互层按所属缝（可多选）/ 构件类型 / 大类过滤构件表，
  并联动 three.js 预览高亮对应构件；筛选仅影响显示，导出始终全量
  （研究可复现性优先；「导出子集」另行评估）[P1]
- ⬜ 节点式 3D 编辑（V2）[P2]
...
💡 **单位制扩展与一键换算**（G14）：
- 输入单位可选（mm / 营造尺 / 公分 / 英寸），用户可自定义「单位 → 公制」比例
  （如 1 营造尺 = 320mm；历代营造尺实长不同，比例必须开放输入）
- 单位与比例写入数据文件，与 schema `$version`（毕业条件 2）同步设计，保证可复现
- **一键换算语义 = 重定标**：已建模项目切换单位/比例后，所有构件尺寸与缝
  坐标按比例重算，相对位置不变（参数化管线下实现 = 重跑 resolver + codegen，
  无需网格缩放）
...
> 注：「构件 ≥3 类」若走枋/槫路径，z 向定位与构件朝向（见「位置语义」区）
> 为硬前置；走内柱/金柱路径则可零前置复用现有圆柱（见开放问题 5）。
